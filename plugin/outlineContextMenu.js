class outlineContextMenuPlugin extends BasePlugin {
    styleTemplate = () => ({
    ...
    appendFirst = () => {
        ...
        document.querySelector("#outline-menu").insertAdjacentHTML("beforeend", html) 
    }
    ...
    appendSecond = () => {
        ...
        const templates = this.config.MENUS.map(({ LIST = [] }, idx) => {
            ...
            return this.ulTemplate({ className: "plugin-outline-menu-second", idx, children })
        })
        ...
        const templates = this.config.MENUS.flatMap(({ LIST = [] }, idx) => {
            return LIST
                ...
                .map(plugin => {
                    ...
                    return { className: "plugin-outline-menu-third", "data-plugin": plugin.fixedName, idx, children }
                })
                .map(this.ulTemplate)
        })
        this.utils.entities.eContent.append(...this._createElement(templates))
    ...
    listen = () => {
        ...
        //获取右键点击的标题。  
        let headingNode
        document.querySelector(".sidebar-menu > .sidebar-content > .outline-content").addEventListener("contextmenu", ev => {

            headingNode = File.editor.findElemById( ev.target.getAttribute("data-ref") )
        })

        // click on the first level menu
        $("#outline-menu").on("click", `[data-key="${this.noExtraMenuGroupName}"]`, function () {
            ...
            that.utils.updatePluginDynamicActions(fixedName, headingNode ) //需要传标题节点
            ...
            if (that.groupName === this.dataset.key) {
                ...
                if (document.querySelector(".plugin-outline-menu-second.show")) {
                    document.querySelectorAll(`.plugin-outline-menu-third:not([idx="${idx}"])`).forEach(removeShow)
                }
                document.querySelectorAll(`.plugin-outline-menu-second:not([idx="${idx}"]) .plugin-menu-item.active`).forEach(removeActive)
                document.querySelectorAll(`.plugin-outline-menu-second:not([idx="${idx}"])`).forEach(removeShow)
                that.showMenuItem(document.querySelector(`.plugin-outline-menu-second[idx="${idx}"]`), this)
                this.classList.add("active")
            } else {
                document.querySelectorAll(`#context-menu li[data-key="${that.groupName}"]`).forEach(removeActive)
                document.querySelectorAll(".plugin-outline-menu-second, .plugin-outline-menu-third").forEach(removeShow)
            }
        })

        // Display the third level menu
        $(".plugin-outline-menu-second").on("mouseenter", "[data-key]", function () {
            document.querySelectorAll(".plugin-outline-menu-third").forEach(removeShow)
            ...
            const third = document.querySelector(`.plugin-outline-menu-third[data-plugin="${fixedName}"]`)
            const noStaticActions = third && third.children.length === 0
            let dynamicActions = that.utils.updatePluginDynamicActions(fixedName, headingNode ) //需要传标题节点
            ...
            if (this.querySelector('span[data-lg="Menu"]')) {
                ...
                removeActive(document.querySelector(".plugin-outline-menu-second .has-extra-menu"))
            }
            ...
            if (action) {
                ...
                if (plugin.call) {
                    plugin.call( headingNode[0] ) //需要传标题节点
                }
            }
            that.hideMenuIfNeed()
        })
        ...
        $(".plugin-outline-menu-third").on("click", "[data-key]", function () {
    ...
    call = async action => {
        const callMap = {
            ...
            toggle_hotkey: () => {
                ...
                document.querySelectorAll(".plugin-outline-menu-second .ty-menu-shortcut, .plugin-outline-menu-third .ty-menu-shortcut").forEach(toggle)
            },
        }
...
module.exports = {
    plugin: outlineContextMenuPlugin
}
